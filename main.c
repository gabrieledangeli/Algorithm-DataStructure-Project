...
int get_valid_command(char command[]);
    ...
    while (command[i] != 'q')
    {
        ...
        case 'c':
            ...
            else
            {
                ...
                InsertInHistory(document, history);
                command_c(document, command, history);
            }
            break;
        case 'd':
...
int get_valid_command(char command[])
{
    ...
    while (not_valid)
    {
        ...
        command_ok = 1;
        for (command_index = 0; (command[command_index] < 'a' || command[command_index] > 'z') && command_ok; command_index++)
        {
            if (command[command_index] == '\n')
            {
                printf("Error: Command type not specified.\n");
                printf("Please, type again your command: ");
                command_ok = 0;
            }
        }
        if (command[command_index] >= 'a' && command[command_index] <= 'z')
            not_valid = 0;
    }
